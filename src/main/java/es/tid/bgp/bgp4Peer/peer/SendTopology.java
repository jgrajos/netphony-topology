...
public class SendTopology implements Runnable {
			...
			if (!(bgp4SessionsInformation.getSessionList().isEmpty())){
				...
				if (sendIntraDomainLinks){//Intradomain Links
					...
					while (iter.hasMoreElements()){						
						...
						sendITNodeNLRI( domainID, ted.getItResources());
					}
				}
	...
	private void sendITNodeNLRI(Inet4Address domainID, IT_Resources itResources){
		System.out.println(" --- ---------------------------sendITNodeNLRI");
		System.out.println(" --- ---------------------------domainID: "+domainID);
		System.out.println(" --- ---------------------------cpu: "+itResources.getCpu());
		System.out.println(" --- ---------------------------mem: "+itResources.getMem());
...
//		}
	 ...
	 * @param node_info
	 */
	private  BGP4Update createMsgUpdateITNodeNLRI(Inet4Address domainID, IT_Resources itResources){
		try{
	
			System.out.println(" --- ---------------------------createMsgUpdateITNodeNLRI");
			BGP4Update update= new BGP4Update();	
			//Path Attributes
			ArrayList<PathAttribute> pathAttributes = update.getPathAttributes();
			//Origin
			...
			itNodeNLRI.setNodeId(domainID.getHostAddress());
